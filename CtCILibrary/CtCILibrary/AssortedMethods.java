...
public class AssortedMethods {
	...
	// Random Array 만들기   // 자바에서 함수 만들때 공용성여부,
	// static 여부  return value (simple val or array(1차 혹은 2차), type  순으로 사고의 틀 정리
	public static int[] randomArray(int N, int min, int max) {  // min, max 사이의 수 중에서 N개 생성
		int[] array = new int[N];
		for (int j=0;j<N;j++){
	...
	/* Create ree by mapping the array left to right, top to bottom */
	public static TreeNode createreeFromArray(int[] array){
		if ( array.length > 0) {
